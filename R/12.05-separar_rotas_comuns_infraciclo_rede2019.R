...
# pasta_osmciclo_19 <- sprintf("%s/E_2019_osm_way_ids_aop_ciclo", pasta_aoprv_teste)
# dir.create(pasta_osmciclo_19, recursive = TRUE, showWarnings = FALSE)
...
out_file <- sprintf('%s/03_ttmatrix_inicial_teste_hexagono_ZL_%s_infraciclo.csv', pasta_aoprv_teste, ano)
...
ttmatrix <- sprintf('%s/03_ttmatrix_inicial_teste_hexagono_ZL_%s_infraciclo.csv', pasta_aoprv_teste, ano)
...
# saida %>% filter(infra_ciclo > 0) %>% sample_n(20)

out_file <- sprintf('%s/04_ttmatrix_final_teste_hexagono_ZL_%s_infraciclo.csv', pasta_aoprv_teste, ano)
...
ids_file_out <- sprintf('%s/05_osm_way_ids_aop_ciclo_%s.csv', pasta_aoprv_teste, ano)
