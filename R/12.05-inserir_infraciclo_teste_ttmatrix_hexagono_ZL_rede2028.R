...
calcular_dist_ciclo <- function(trip_id_alt) {
  ...
  trip_osm_ids <- list.files(pasta_osmids_aopt2, pattern = trip_id_alt, recursive = FALSE, full.names = TRUE)
...
ciclo_comuns <- rbind(ciclo_comuns, ciclo_ciclov_semsem) %>% distinct()
ciclo_comuns <- ciclo_comuns %>% mutate(infra_ciclo = 'ciclo_comum')


# Juntar tudo em um único dataframe
ciclo_2019 <- rbind(ciclo_expressas, ciclo_ciclofx, ciclo_comuns)
rm(ciclo_expressas, ciclo_ciclofx, ciclo_comuns, ciclo_ciclov_semsem)

# Tem algumas estruturas duplicadas
# ciclo_2019 %>% group_by(osm_id) %>% tally() %>% filter(n >1)
# ciclo_2019 %>% filter(osm_id == '198576059')
ciclo_2019 <- ciclo_2019 %>% distinct(osm_id, .keep_all = TRUE)
# viario_sp %>% filter(osm_id %in% ciclo_2019$osm_id) %>% mapview()
...
ciclo_futura <- ciclo_futura %>% filter(!osm_id %in% ciclo_2019$osm_id)

# Juntar tudo em um único dataframe
infra_ciclo <- rbind(ciclo_2019, ciclo_futura) %>% distinct()

# Checar - ficou algum osm_id repetido?
# infra_ciclo %>% group_by(osm_id) %>% tally() %>% filter(n > 1)
rm(ciclo_2019, ciclo_futura)
...
rotas_m <- sprintf('%s/03_ttmatrix_teste_hexagono_ZL_2028.csv', pasta_aoprv_teste)
...
out_file <- sprintf('%s/04_ttmatrix_teste_hexagono_ZL_2028_infraciclo.csv', pasta_aoprv_teste)
if (file.exists(out_file)) {
