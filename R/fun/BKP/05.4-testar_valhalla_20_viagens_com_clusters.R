...
processar_viagens <- function(sel_trip, df_trips) {
  ...
  # ----------------------------------------------------------
  # Testes de clusterização com st_dbscan
  # ----------------------------------------------------------
  
  # Reconhecer e retirar paradas com o st_dbscan()
  this <- st_dbscan(x    = viagem$lon,
                    y    = viagem$lat, 
                    time = viagem$timestamps, 
                    eps1 = 0.0001, # ~11 m
                    eps2 = 180, # Artigo Facing the needs for clean bicycle data
                    minpts = 18, # GPS Yellow em 1 ponto a cada 5 seg -> 18 equivale a ~90 seg; se considerar a cada 3.75s, é 24 pontos
                    dry  = TRUE) %>% 
          as.data.frame()
  
  
  lala <- viagem %>% cbind(this)
  lala %>% st_as_sf(coords = c('lon', 'lat'), crs = 4326) %>% mapview(zcol = 'cluster', cex = 3)
  
  viagem <- lala %>% filter(cluster == 0) %>% select(-cluster)
  ...
  while (condicao == TRUE) {
    ...
    if (n_row_antes == nrow(viagem)) { condicao = FALSE }
    
  }

  print(mean(viagem$time_s))
  ...
  # ----------------------------------------------------------
  # TODO - Avaliar viagens suspeitas, com iteração > 2
  ...
  # TODO - Só processar viagens com quantidade mínima de pontos
  i <- 4
  if (qtd_quebras > 0) {
