# A Rede de Referência 2028 é a rede 2024 somada às marcações de 'referencia'
# no shapefile
...
pasta_redes_testes  <- sprintf("%s/testes", pasta_redes_24_28)
...
ciclo2024 <- sprintf('%s/sao_paulo_osm_filtrado_com_qgis_id_redes_cicloviarias_2019_2024_2028.gpkg', pasta_redes_24_28)
ciclo2024 <- read_sf(ciclo2024)
head(ciclo2024)

# Da rede de 2024, quais são as ciclovias? São as que estão marcadas como 'sim'
# no shape com as marcações de 2024 ou as que são da rede 2019 e aparecem na 
# seleção de osm_ids em 01_atributos_ciclovias_comuns_2019.csv
ciclo_ciclov24 <- 
  ciclo2024 %>% 
  # Primeiro, manter somente o que pertence à rede 2024
  filter(!is.na(rede_cicloviaria_2024)) %>% 
  filter(ciclovia_2024 == 'sim' | (osm_id %in% ciclo_comuns$osm_id & rede_cicloviaria == '2019')) %>%
...
# Testar resultados
# st_write(ciclo_ciclov24, sprintf('%s/teste_ciclovias.gpkg', pasta_redes_testes), driver = 'GPKG', append = FALSE)
...
# Testar resultados
# st_write(ciclo_ciclofx24, sprintf('%s/teste_ciclofaixas.gpkg', pasta_redes_testes), driver = 'GPKG', append = FALSE)
...
ciclo2024 %>% filter(osm_id == '264984110')
...
# Testar resultados
# st_write(ciclo_2028, sprintf('%s/teste_rede2028_sem_2024.gpkg', pasta_redes_testes), driver = 'GPKG', append = FALSE)


# Limpar ambiente
rm(ciclo_expressas, ciclo_comuns, ciclo2024)
...
osm_edit <-
  osm_edit %>%
  mutate(tag4_lcn = ifelse((osm_id %in% ciclo_ciclofx24$osm_id | osm_id %in% ciclo_2028$osm_id), 
...
pbf_file_out  <- sprintf('%s/20220216_sao_paulo_edited_20250302_B_infraciclo_2028_referencia.osm.pbf', pasta_redes_24_28)
