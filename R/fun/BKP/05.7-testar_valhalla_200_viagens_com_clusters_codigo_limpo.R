...
processar_viagens <- function(sel_trip, df_trips) {
  ...
  if (qtd_quebras == 0) {
    ...
    processar_trecho(viagem, sel_trip, cp_a2, cp_viagem, tempo_min_viagem, 
    ...
    for (i in seq(1:qtd_quebras)) {
      ...
      processar_trecho(trecho, sel_trip, cp_a2, cp_viagem, tempo_min_viagem, 
                       qtd_min_pontos, dist_min_viagem, active_mode = 'pedestrian',
                       pontos_elevacao, vias_ciclo, vias_restritas, semaforos,
                       pasta_viagens_gpkg, pasta_viagens_pngs, 
                       pasta_viagens_csv1, pasta_viagens_csv2) 
...
rotas_de_teste <- yellow_sp %>% sample_n(200) %>% select(trip_id) %>% distinct() %>% arrange(trip_id)
