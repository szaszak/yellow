...
pasta_tmp_osmids  <- sprintf("%s/X_osm_way_ids_tmp", pasta_aoprv_teste)
...
for (line_id in ids_origem$id_orig) {
  ...
  if (ano == '2019') {
    osm_ids <- sprintf('%s/%s.csv', pasta_osmids_aopt, line_id)
  } else if (ano == '2028') {
    osm_ids <- sprintf('%s/%s.csv', pasta_osmids_aopt2, line_id)
  }
...
rm(ids_origem, line_id, osm_ids, rotas_ciclo, infra_ciclo, rotas_vias_comuns, 
...
dist_ciclo_sep_osmids <- function(trip_id_alt) {
  ...
  if (ano == '2019') {
    trip_osm_ids <- list.files(pasta_osmids_aopt, pattern = line_id, recursive = FALSE, full.names = TRUE) 
  } else if (ano == '2028') {
    trip_osm_ids <- list.files(pasta_osmids_aopt2, pattern = line_id, recursive = FALSE, full.names = TRUE) 
  }
...
for (i in rotas_ciclo$alt_id) { dist_ciclo_sep_osmids(i) }
