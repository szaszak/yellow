...
pasta_viagens_gpkg <- sprintf("%s/viagens_processadas_gpkg", pasta_map_matching)
pasta_viagens_pngs <- sprintf("%s/viagens_processadas_pngs", pasta_map_matching)
dir.create(pasta_viagens_csv, recursive = TRUE, showWarnings = FALSE)
dir.create(pasta_viagens_log, recursive = TRUE, showWarnings = FALSE)
...
log_file <- sprintf('%s/%s.csv', pasta_map_matching, log_base_name)

# Se arquivo não existe, criar um header em um novo arquivo
if (!log_file %in% list.files(pasta_map_matching, pattern = '^viagens_processadas_log.csv', full.names = TRUE)) {
  ...
  file.copy(log_file, sprintf('%s/%s_BKP_%s.csv', pasta_map_matching, log_base_name, ts))
}
...
# open_file <- sprintf('%s/sp_20_viagens_teste.rds', pasta_map_matching)
...
processar_viagens <- function(sel_trip, df_trips, salvar_gpkgs = TRUE, salvar_pngs = TRUE) {
  ...
  } else if ((qtd_quebras >= 5 & qtd_iteracoes >= 5) | qtd_quebras > 25) {
...
# (start = Sys.time())
# walk(rotas_de_teste$trip_id, processar_viagens, yellow_sp, salvar_gpkgs = TRUE, salvar_pngs = FALSE)
# Sys.time() - start


# ----------------------------------------------------------
# Testar com um mês inteiro
# ----------------------------------------------------------
