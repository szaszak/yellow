...
plot(predict(ols_resultados), base_modelo$vel_med_m5, 
     xlab = "predicted", 
     ylab = "observed",
     main = 'Modelo 1 - Valores Previstos vs. Observados',
     cex = 0.2)
...
# # ------------------------------------------------------------------------------
# # Modelo: OLS com regressão simples com erros robustos
# # ------------------------------------------------------------------------------
...
out_png <- sprintf('%s/modelo_velmed5_histogram.png', pasta_graficos)
...
out_png <- sprintf('%s/modelo_velmed5_boxplot.png', pasta_graficos)
png(out_png, width = 600, height = 350)
...
for (i in seq(1, 5)) {
  ...
  out_png <- sprintf('%s/modelo_velmed5_plot%i.png', pasta_graficos, i)
...
scatter.smooth(
  ...
  main = bquote("Plot of standardized residuals versus fitted"))
