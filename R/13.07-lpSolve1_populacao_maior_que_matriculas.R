...
# cada hexágono de origem e destino (relação população vs matrículas) - neste
# script, a população não é ajustada, de forma que há mais população de interesse
# do que  matrículas disponíveis. Com isso, a aplicação do lpSolve é invertida:
# a população é tratada como oferta e as matrículas como demanda. O problema
# é que os resultados vão apresentar hexágonos em que a população registra viagens
# em 2019 mas não em 2028 (159 linhas), significando que os comparativos de 
# cenário não usariam exatamente as mesmas origens e destinos.
...
dir.create(pasta_opaop_ano, recursive = TRUE, showWarnings = TRUE)
...
st_write(hex_impossiveis, out_file6, driver = 'GPKG', append = FALSE)
...
cost.mat <- 
  cost.mat %>% 
  # Aqui, como a população é MAIOR do que a quantidade de matrículas, os 
  # destinos (oferta) ficam como COLUNAS; origens (demanda) ficam como LINHAS
...
# ------------------------------------------------------------------------------
# Etapa 3 - lpSolve - Demora entre 1h40 e 3h para rodar
# ------------------------------------------------------------------------------
...
out_file1 <- sprintf('%s/01_lpsolve_solution_%s_%smin.csv', pasta_opaop_ano, ano, tempo_max)
write_delim(as.data.frame(solution$solution), out_file1, delim = ';')
...
out_file2 <- sprintf('%s/02_lpsolve_resultados_viagens_por_par_OD_%s_%smin.csv', pasta_opaop_ano, ano,tempo_max)
write_delim(resultados, out_file2, delim = ';')
