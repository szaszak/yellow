...
# gera os resultados com nomes também baseados nos ids de origem. 
# Já filtradas as rotas com 15 min e com menor weight, este script demora:
# Cenário 2024: 20 minutos
# Cenário 2028: 65 minutos
# Nos dois cenários, usando 6 instâncias do RStudio rodando em paralelo. Isso 
# é possível porque ocupa pouco processamento e RAM, então dá para abrir vários 
# ao mesmo tempo - no Jupyter não consegui fazer um processo com future que 
# funcionasse de forma satisfatória. O resultado é a ttmatrix inicial das rotas 
# que passaram por infra cicloviária. Em seguida, essa ttmatrix é juntada à das
# rotas que só passaram por vias comuns, gerando a ttmatrix completa.
...
# ano <- '2024'; min_thres <- 15 #; sec_thres <- min_thres * 60
...
# somar às existentes
pasta_tmp_ttmatrix <- sprintf("%s/Z_%s_tmp_ttmatrix", pasta_ttmatrix_24_28, ano)
...
dist_ciclo_map <- function(rotas_ciclo) {
  ...
  if (ano == '2024') {
    trip_osm_ids <- list.files(pasta_ids_aopt_24, pattern = base_id, recursive = FALSE, full.names = TRUE) 
  } else if (ano == '2028') {
  ...
  if (nrow(rotas_ciclo) == length(viario_rotas_cropped)) {
    ...
    viario_rotas_cropped <- 
      ...
      map(function(x) {
        ...
        for (i in c('ciclo_expressa', 'ciclovia_comum', 'ciclofaixa', 'via_comum')) {
          if (!i %in% names(x)) { x <- x %>% mutate(!!i := 0) }
        }
        
        # Somar extensões percorridas em infra cicloviária
        x <- x %>% mutate(infra_ciclo = ciclo_expressa + ciclovia_comum + ciclofaixa)
        # Reordenar colunas
        x <- x %>% select(alt_id, via_comum, infra_ciclo, ciclo_expressa, ciclovia_comum, ciclofaixa)
    ...
    rotas_ciclo <- 
      ...
      relocate(c(via_comum, infra_ciclo, ciclo_expressa, ciclovia_comum, ciclofaixa), 
               .before = 'poly') %>% 
...
# Abrir cópia do viário de SP com osm_ids (copiado da pasta 
# /yellow_dados/12_aop_revisitado/03_alternatives_2019_2028/)
viario_sp <- read_sf(sprintf('%s/tmp_sao_paulo_osm_filtrado.gpkg', pasta_ttmatrix_24_28))
head(viario_sp)

# Abrir infra cicloviária
infra_ciclo <- sprintf('%s/tmp_infra_ciclo_%s.csv', pasta_ttmatrix_24_28, ano)
...
# Definir arquivo de saída
if (ano == '2024') {
  out_file <- sprintf('%s/08_tmp_ttmatrix_%s_rotas_infra_ciclo_%smin_menor_peso.csv', pasta_ttmatrix_24_28, ano, min_thres)
} else if (ano == '2028') {
  out_file <- sprintf('%s/09_tmp_ttmatrix_%s_rotas_infra_ciclo_%smin_menor_peso.csv', pasta_ttmatrix_24_28, ano, min_thres)
}
...
# ------------------------------------------------------------------------------
# Juntar ttmatrix iniciais (vias comuns e vias com infra cicloviária)
# ------------------------------------------------------------------------------

library('tidylog')
if (ano == '2024') {
  tt_ini_vias_comuns  <- sprintf("%s/06_tmp_ttmatrix_%s_rotas_vias_comuns_%smin_menor_peso.csv", pasta_ttmatrix_24_28, ano, min_thres)
  tt_ini_vias_ciclos  <- sprintf("%s/08_tmp_ttmatrix_%s_rotas_infra_ciclo_%smin_menor_peso.csv", pasta_ttmatrix_24_28, ano, min_thres)
} else if (ano == '2028') {
  tt_ini_vias_comuns  <- sprintf("%s/07_tmp_ttmatrix_%s_rotas_vias_comuns_%smin_menor_peso.csv", pasta_ttmatrix_24_28, ano, min_thres)
  tt_ini_vias_ciclos  <- sprintf("%s/09_tmp_ttmatrix_%s_rotas_infra_ciclo_%smin_menor_peso.csv", pasta_ttmatrix_24_28, ano, min_thres)
}
...
# Gerar ttmatrix final
ttmatrix <- rbind(tt_ini_vias_ciclos, tt_ini_vias_comuns) %>% arrange(hex_id)
...
# Gravar resultados
if (ano == '2024') {
  out_file <- sprintf('%s/10_ttmatrix_%s_res09_%smin_menor_peso.csv', pasta_ttmatrix_24_28, ano, min_thres)
} else if (ano == '2028') {
  out_file <- sprintf('%s/11_ttmatrix_%s_res09_%smin_menor_peso.csv', pasta_ttmatrix_24_28, ano, min_thres)
}
