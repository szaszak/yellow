...
gh_route_alt <- function(hex_id, route_options) {
  ...
  if (http_status(gh_response)$message == 'Success: (200) OK') {
    ...
    for (i in seq(1, length(paths$details$osm_way_id))) {
      ...
      if (nrow(osm_ways) > 0) {
        ...
        # Gravar resultados
        osm_way_out <- sprintf('%s/%s_%i.csv', pasta_osmids_aopt2, df_line$id, i)
  ...
  tmp_file <- sprintf('%s/%s_modalt.csv', pasta_rotas_aopt2, df_line$id)
...
# Arquivo de saída
out_file <- sprintf('%s/03_ttmatrix_teste_hexagono_ZL_2028.csv', pasta_aoprv_teste)
